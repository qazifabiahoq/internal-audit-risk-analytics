...
**kagglehub**
Used to load the audit finding dataset directly from Kaggle using the PANDAS adapter. The pipeline requires real Kaggle API credentials (`KAGGLE_USERNAME` / `KAGGLE_KEY`) and fails loudly if they're missing or the download fails — there is no synthetic fallback, so every number on the dashboard comes from the real dataset.
...
The dashboard reads static JSON files in `public/data/`, generated by `pipeline/analysis.py` against the real Kaggle dataset. There is no synthetic fallback — if Kaggle can't be reached or credentials aren't set, the pipeline errors out instead of producing fabricated numbers.

To refresh the data:

1. Get a Kaggle API token from [kaggle.com/settings](https://www.kaggle.com/settings) -> API -> Create New Token.
2. In this repo's GitHub Settings -> Secrets and variables -> Actions, add `KAGGLE_USERNAME` and `KAGGLE_KEY` from that token.
3. Run the **Refresh dashboard data from Kaggle** workflow (Actions tab -> Refresh dashboard data from Kaggle -> Run workflow). It re-runs the full pipeline against live Kaggle data and commits the updated `public/data/*.json` directly.

To run the pipeline locally instead, set `KAGGLE_USERNAME` and `KAGGLE_KEY` as environment variables (or place a `kaggle.json` at `~/.kaggle/kaggle.json`) and run `python pipeline/analysis.py`.
